...
export class ManageHours implements OnInit {
  ...
  submit() {
    ...
    this.http.put(`${environment.apiUrl}/business/hours`, { dias }).subscribe({
      ...
      error: (e) => {
        this.error = e.error?.message || 'Error al guardar horarios';
        this.saving = false;
      }
    });
  }
}
EOFcd "/home/sistema/Escritorio/nuevo barber/UnidosBarberShop/unidos-barber-frontend"

cat > src/app/features/admin/manage-hours.ts << 'EOF'
import { Component, OnInit, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ReactiveFormsModule, FormBuilder, FormArray, FormGroup, Validators } from '@angular/forms';
import { RouterLink } from '@angular/router';
