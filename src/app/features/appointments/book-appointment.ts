...
export class BookAppointment implements OnInit {
  ...
  selectService(s: Service) {
    this.form.patchValue({ serviceId: s.id, time: '' });
    this.selected.set(s);
    this.slots.set([]);
  }
  ...
  onDateChange() {
    ...
    this.apptSvc.getBusinessHours().subscribe((res: any) => {
      ...
      // Obtener TODAS las citas del día (admin ve todas, cliente ve las suyas)
      this.apptSvc.getAll().subscribe((apptRes: any) => {
        const allAppts = (apptRes.data ?? apptRes);
        const occupied = allAppts.filter((a: any) =>
          a.estado !== 'CANCELADA' &&
          new Date(a.startDatetime).toISOString().split('T')[0] === date
        );
        ...
        while (new Date(cur.getTime() + dur * 60000) <= end) {
          ...
          const isOccupied = occupied.some((a: any) => {
  ...
  submit() {
    ...
    this.apptSvc.create({
      serviceId, startDatetime: time, notas,
      userId: user!.id, duracionMin: this.selected()!.duracionMin
    }).subscribe({
